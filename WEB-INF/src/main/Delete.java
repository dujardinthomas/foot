...
@WebServlet("/servlet-delete") // nom du fichier en url
...
public class Delete extends HttpServlet {
	...
	public void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		// </head><body><center>");
		out.println("<h1>Suppression d'une donnée</h1>");
		...
		if (p == null) {
			...
			if (p.getRole().equals("admin") || p.getRole().equals("capitaine")) {
				if (((table.equals("joueurs")) || (table.equals("equipes")) || (table.equals("rencontres")))
					...
					try { // CREATION DE LA REQUETE AVEC LES COLONNES
						String querySuppr = "DELETE FROM " + table + " WHERE ";
						for (int i = 1; i <= numberOfColumns; i++) {
							querySuppr = querySuppr + rsmd.getColumnName(i) + " = ? ";
							if (i != numberOfColumns) {
								querySuppr = querySuppr + " AND ";
							}
						}

						out.println("requete de suppression : " + querySuppr);
						PreparedStatement requete = con.prepareStatement(querySuppr);
						...
						for (int i = 1; i <= numberOfColumns; i++) {
							...
							// tout ce qui est pas int devient string
								...
								out.println("insertion string ok");
							}
						}
						...
						out.println("<h2>SUPPRESSION REUSSIE</h2>");
